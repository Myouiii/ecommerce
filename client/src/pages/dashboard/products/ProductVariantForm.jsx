...
export default function ProductVariantForm({ currentVariant, currentProductId, open, handleClose, handleCreateDone }) {
  ...
  const NewProductSchema = Yup.object().shape({
    variantName: Yup.string()
      .required(t('products.variant-validation'))
      .min(6, t('dashboard.products.name-validation-len'))
      .max(25, t('dashboard.products.name-validation-len')),
    sku: Yup.string().required(t('products.sku-validation')),
    price: Yup.number()
      .required(t('products.price-validation'))
      .min(1000, t('products.market-price-validation1'))
      .max(10000000000, t('products.market-price-validation2')),
    marketPrice: Yup.number()
      .required(t('products.market-price-validation'))
      .min(1000, t('products.market-price-validation1'))
      .max(10000000000, t('products.market-price-validation2')),
    quantity: Yup.number()
      .required(t('products.number-validation'))
      .min(1, t('products.number-validation1'))
      .max(9999, t('products.number-validation2'))
  });
  ...
  return (
    <Dialog open={open} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        <Stack spacing={1} sx={{ marginBottom: theme.spacing(2) }}>
          {/* {errors &&
