...
export default function UserFormEdit() {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      avatar: '',
      isVerified: user?.status === USER.STATUS.ACTIVE || false,
      status: user?.status,
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      try {
        if (userAdmin && !currentUser) {
          dispatch(updateUser(userAdmin?._id, values));
        } else {
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  {user?.role !== undefined && (
                    <Autocomplete
                      ...
                      options={userAdmin.role === 'admin' ? USER.ROLE : USER.ROLE.filter((x) => x !== USER.ROLE[0])}
