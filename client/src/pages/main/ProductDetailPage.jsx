...
import { getListRelated, getProductById } from '../../redux/slices/productSlice';
...
export default function ProductDetailPage() {
  ...
  const { item: product, listRelated, isLoading, error } = useSelector((state) => state.product);
  ...
  useEffect(() => {
    ...
    if (product?._id) {
      ...
      dispatch(getListRelated('', product?.brand?._id, product?.category?._id, 0, limit));
    }
    window.scrollTo(0, 0);
  ...
  return (
    <Page title={(product?.name?.concat(' - ') || '') + t('home.page-title')} sx={{ paddingTop: 2, paddingBottom: 5 }}>
      <Container maxWidth="lg" sx={{ paddingTop: 2, paddingBottom: 5 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Button
            color="inherit"
            href={`/q?c=${product?.category?.slug}&b=${product?.brand?.slug}`}
            endIcon={<Icon icon={arrowIosForwardFill} />}
          >
            {t('common.see-more')}
          </Button>
        </Box>
        <ProductList products={listRelated} isLoading={isLoadingRelated} limit={5} />
      </Container>
    </Page>
  );
}
