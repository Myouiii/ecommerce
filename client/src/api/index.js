...
API.interceptors.response.use(
  ...
  async (error) => {
    ...
    if ((error?.response?.status === 401 || error?.response?.status === 403) && !prevRequest?.sent) {
      ...
      const result = await refreshToken({ refreshToken: rfToken });
...
export const verifyEmail = (email) => API.patch('/auth/verify', { email });
...
export const updateAccountInfo = (formData) => API.patch('/account', formData);
...
export const toggleHideUser = (id) => API.patch(`/users/${id}/toggleHide`);
// ----------------------------User (staff) --------------------------

export const getAllStaffs = () => API.get('/users/staff');
export const getStaff = (identity) => API.get(`/users/staff/${identity}`);
export const createStaff = (formData) => API.post('/users/staff', formData);
...
export const createUser = (formData) => API.post('/users/customer', formData);
...
// ----------------------------Brand-----------------------------------
export const getAllBrand = (fields, isHidden) =>
  fields ? API.get(`/brands?fields=${fields}&isHidden=${isHidden}`) : API.get('/brands');
...
// ----------------------------Static----------------------------
export const getBestSellerProduct2 = () => API.get('/statistic/products/best-seller');
export const getProductsStatic = () => API.get('/statistic/products');
export const getStaMonthlyRevenue = (year = new Date().getFullYear()) =>
  API.get('/statistic/monthly-revenue', { params: { year } });
