...
const maxDate = new Date(Date.now() - 86400000 * 365 * 10); // 10 YEARS AGO FROM NOW
...
export default function AccountGeneral() {
  ...
  const AccountInfoSchema = Yup.object().shape({
    ...
    dob: Yup.date().default(maxDate).min(minDate).max(maxDate).required('Date required'),
    email: Yup.string()
  ...
  const handleDrop = useCallback(
    (acceptedFiles) => {
      ...
      const fileSend = {
        originalname: acceptedFiles[0]?.path,
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={4}>
        <Card sx={{ py: 8, px: 3, textAlign: 'center' }}>
          <UploadAvatar
            accept="image/*"
            file={values.avatarUrl || values.avatar}
            maxSize={3145728}
            onDrop={handleDrop}
