...
const SELECTED_FIELDS = '_id numericId customer user address isReceiveAtStore status paymentMethod paymentStatus items subTotal shippingFee discount total createdAt updatedAt updatedBy';
...
async function getTotal() {
  let result = await Order.aggregate([
    ...
    { $group: { _id: '$user', subTotal: { $sum: '$subTotal' } } },
    { $sort: { "subTotal": -1 } }
  ]).exec();
...
async function create(customerInfo, orderData, createdBy) {
  ...
  if (mongoose.Types.ObjectId.isValid(customerInfo)) {
    ...
    if (!user) {
      throw ApiErrorUtils.simple('User does not exist', 400);
    }
...
async function update(userId, orderId, data) {
  if (!mongoose.Types.ObjectId.isValid(userId)) {
    throw ApiErrorUtils.simple('Invalid user id', 400);
  }
  if (!mongoose.Types.ObjectId.isValid(orderId)) {
    throw ApiErrorUtils.simple('Invalid order id', 400);
  }

  const currentData = await getOne(orderId);
  if (!currentData) {
    throw ApiErrorUtils.simple('Order does not exist', 400);
  }

  if (currentData.status === constants.ORDER.STATUS.CANCELLED) {
    throw ApiErrorUtils.simple('Order has been canceled', 400);
  }

  // if (currentData.status === constants.ORDER.STATUS.COMPLETED) {
  //   throw ApiErrorUtils.simple('Order has been completed', 400);
  // }

  if (data.status === constants.ORDER.STATUS.CANCELLED) {
...
async function cancel(orderId) {
  ...
  try {
    ...
    if (currentData.paymentStatus === constants.ORDER.PAYMENT_STATUS.PAID) {
      throw ApiErrorUtils.simple('Order has been paid', 400);
    }
    if (!currentData.items) {
      throw ApiErrorUtils.simple('Order has no product', 400);
    }

    for (let i = 0; i < currentData.items.length; i++) {
      const orderItem = currentData.items[i];

      const product = await Product.findOne(
        { _id: orderItem.productId, 'variants.sku': orderItem.sku }
      ).lean().exec();

      if (!product || product?.variants?.length === 0) {
        throw new ApiErrorUtils({
          message: 'Product does not exist',
          errors: { productId: orderItem.productId, sku: orderItem.sku },
          status: 404
        });
      }

      const selectedVariant = product.variants.find(variant => variant.sku === orderItem.sku);
      if (!selectedVariant) {
        throw new ApiErrorUtils({
          message: 'Product does not exist',
          errors: { productId: orderItem.productId, sku: orderItem.sku },
      ...
      await Product.findOneAndUpdate(
        { _id: orderItem.productId, 'variants.sku': orderItem.sku },
        {
          $inc: { 'variants.$.sold': -orderItem.quantity }
        },
        { session }
      );
