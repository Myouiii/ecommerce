...
async function googleAuthenticate(payload, ipAddress) {
  ...
  const user = await userService.getOrCreateByGoogleId(googleId, email, firstName, lastName ? lastName : 'User', avatar);
  if (!user) {
...
async function verifyEmail(email) {
  const user = await userService.getOne(email, '-addresses');
  if (!user) {
    throw ApiErrorUtils.simple2(responseDef.AUTH.USER_NOT_FOUND);
  }
  const updateData = {};
...
async function authenticate(username, password, ipAddress) {
  ...
  if (user.status === USER.STATUS.INACTIVE) {
    throw ApiErrorUtils.simple2(responseDef.AUTH.USER_INACTIVE);
  }
  if (user.status === USER.STATUS.LOCKED) {
    throw ApiErrorUtils.simple2(responseDef.AUTH.USER_LOCKED);
  }
  // authentication successful so generate jwt and refresh tokens
  const jwtToken = JwtUtils.generateToken({ _id: user._id });
  const refreshToken = generateRefreshToken(user._id, ipAddress);
...
async function getRefreshToken(token) {
  const refreshToken = await RefreshToken
    .findOne({ token: token?.refreshToken })
...
async function sendOrderViaMail(email, orderSend, language) {
  if (!StringUtils.isEmailAddress(email)) {
    throw ApiErrorUtils.simple2(responseDef.AUTH.INVALID_EMAIL);
  }
  const isExistUser = await userService.isExistEmail(email);
  ...
  await mailerService.sendWithOrderTemplate(email, orderSend, language);
  return true;
}
