...
const formatResult = (product, sku, qty) => {
  ...
  const variantInfo = variants.find(v => v.sku === sku && v.quantity - v.sold > 0);

  if (variantInfo) {
    return {
      productId: uuId,
      sku,
      qty: qty > variantInfo.quantity - variantInfo.sold ? variantInfo.quantity - variantInfo.sold : qty,
      ...otherProductInfo,
      ...variantInfo
    };
  }
...
async function getCartItemsFromData(items) {
  const filters = { '_id': { $in: items.map(i => i.productId) } };
  ...
  console.log('result: ', result.filter(i => i));
...
async function addItem(userId, productId, sku, qty = 1) {
  ...
  if (!userCart) {
    ...
    if (itemInCart) {
      if (variant.quantity - variant.sold < qty + itemInCart.qty) {
        itemInCart.qty = variant.quantity - variant.sold;
      }
      else {
        itemInCart.qty += qty;
      }
