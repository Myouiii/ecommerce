...
import constants, { USER } from '../constants.js';
...
async function create(data) {
  const user = new User({
    _id: new mongoose.Types.ObjectId(),
    firstName: data.firstName || data?.name, lastName: data.lastName || 'userTest',
    ...data
  });
  return user.save();
}
 ...
 * @returns status user
 */
async function toggleHideUser(identity) {
  const isUUID = StringUtils.isUUID(identity);
  let filter = {}
  if (isUUID) {
    filter = { _id: identity }
  }
  const user = await User.findOne(filter).select('status').lean().exec();
  if (user.status === USER.STATUS.LOCKED) {
    return User.findOneAndUpdate(filter, { $set: { status: USER.STATUS.ACTIVE } }, { new: true, lean: true, fields: '_id status' });
  }
  else if (user.status === USER.STATUS.ACTIVE) {
    return User.findOneAndUpdate(filter, { $set: { status: USER.STATUS.LOCKED } }, { new: true, lean: true, fields: '_id status' });
  }
  else {
    return user;
  }
