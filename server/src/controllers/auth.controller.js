...
export const register = async (req, res, next) => {
  try {
    ...
    if (account) {
      let suffixError =
        account.googleId
          ? `bởi đăng nhập với google`
          : '';
      let error = `Email đã được sử dụng ${suffixError} !`;

      if (account) throw ApiErrorUtils.simple(error, 400);
    }
...
export const sendOrder= async (req, res, next) => {
  try {
    const email = req.body?.email ?? '';
    const language = req.headers['accept-language'];
    await authService.sendOrderViaMail(email, language);
    ResponseUtils.status200(res, 'Send Order success');
  } catch (err) { next(err) }
};
